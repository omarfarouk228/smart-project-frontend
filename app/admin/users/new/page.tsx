  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 py-4 border-b border-border/50 flex items-center gap-3 bg-background/80 backdrop-blur-sm sticky top-0 z-10">
        <Link href="/admin/users" className={cn(buttonVariants({ variant: 'ghost', size: 'icon' }), 'h-8 w-8 shrink-0')}>
          <ArrowLeft className="h-4 w-4" />
        </Link>
        <div className="flex-1 min-w-0">
          <h1 className="text-[16px] font-semibold tracking-tight">Inviter un membre</h1>
      ...
      <div className="flex-1 overflow-y-auto p-6 lg:p-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          ...
          {/* Left — form */}
          <form id="invite-form" onSubmit={handleSubmit((d) => mutation.mutate(d as FormData))} className="space-y-4">
          ...
          <div className="rounded-xl border border-border/60 bg-card p-5 space-y-3">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Rôles</p>
